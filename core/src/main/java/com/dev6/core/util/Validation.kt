...
class Validation {
    val idAlphaNumberPattern: String = "^(?=.*[a-z])(?=.*[0-9])[a-z[0-9]]{5,20}$" // 영어 숫자 섞어쓰기 가능 대문자 X
    val idOnlyAlphabetPattern: String = "^[a-zA-Z]{5,20}$" // 영문만 가능
    ...
    fun checkIdPattern(Id: String): Boolean {
        var result3 = false
        val pattern = Pattern.compile(idAlphaNumberPattern)
        val matcher = pattern.matcher(Id)
        val pattern2 = Pattern.compile(idOnlyAlphabetPattern)
